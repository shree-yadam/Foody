...
const getPastOrderAndItems = (customer_id) => {
  const queryString = `
    SELECT orders.id AS id, menu_items.name AS name, order_items.quantity AS quantity, orders.order_date AS date, total_price AS total
    FROM orders
    JOIN order_items ON orders.id = order_id
    JOIN menu_items ON menu_item_id = menu_items.id
    WHERE customer_id = $1 AND order_status = 'completed'
